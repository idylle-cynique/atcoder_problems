''' ABC259 - D
    ...
    どうにかして各円の通路(円周)の接続関係を整理して隣接リストで表せないか考えてみるところから始めてみる
    ...
    あとはこれを、浮動小数点数計算などによる誤差などを発生させないように計算しながら実装し、
    各円の接続関係を隣接リストの形で表現する

    そこからさらにグラフ探索アルゴリズムのひな型に隣接リスト、スタート位置に相当する円、
    ゴール位置に相当する円とを与え、実際に探索を行わせる
    探索が修了したら、あとは到達済みリスト(このコードにおけるlistオブジェクトのvisited)を取得
    その中のゴール位置に対応する値が到達済み(True)となっているかチェックし、値を解とすればよい

    なお、解として出力したいのは単に行き来可能かどうかの情報だけで、最短距離などではないので
